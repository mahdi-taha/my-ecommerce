document.addEventListener('DOMContentLoaded', function () {
    ...
    if (
        ...
        function initializeBundleOptionFields(fields) {
            const typeSelect = fields.querySelector('.bundle-option-type');
            const selectionLimits = fields.querySelectorAll('.bundle-selection-limit');

            function updateSelectionLimits() {
                const supportsMultiple = ['checkbox', 'multiselect'].includes(typeSelect?.value);

                selectionLimits.forEach(function (container) {
                    const input = container.querySelector('input');

                    container.classList.toggle('d-none', !supportsMultiple);

                    if (input) {
                        input.disabled = !supportsMultiple;
                        input.required = supportsMultiple;
                    }
                });
            }

            typeSelect?.addEventListener('change', updateSelectionLimits);
            updateSelectionLimits();
        }

        document.querySelectorAll('.bundle-option-fields').forEach(initializeBundleOptionFields);

        const bundleEditor = document.getElementById('bundle-options-editor');
        const optionTemplate = document.getElementById('bundle-option-template');
        const itemTemplate = document.getElementById('bundle-item-template');
        let bundleOptionSequence = 0;
        let bundleItemSequence = 0;

        function initializeBundleSelect(select) {
            $(select).select2({ placeholder: 'Select Product', width: '100%' });
        }

        function markDeleted(row, deletedInputClass) {
            const deletedInput = row.querySelector(deletedInputClass);
            const hasExistingId = Boolean(row.querySelector('input[name$="[id]"]'));
            if (!hasExistingId) {
                row.remove();
                return;
            }
            ...
            row.querySelectorAll('input, select, textarea, button').forEach(function (control) {
                if (control !== deletedInput && !control.name?.endsWith('[id]')) control.disabled = true;
            });
            row.classList.add('d-none');
        }

        function setNestedRowValues(row, values) {
            Object.entries(values).forEach(function ([name, value]) {
                if (name === 'items' || name === 'id' || name === 'deleted') return;
                const controls = row.querySelectorAll(`[name$="[${name}]"]`);
                const control = controls[controls.length - 1];
                if (!control) return;
                if (control.type === 'checkbox') control.checked = String(value) === '1';
                else control.value = value ?? '';
            });
        }

        function appendBundleItem(optionRow, optionKey, itemKey, values = {}) {
            const wrapper = document.createElement('div');
            wrapper.innerHTML = itemTemplate.innerHTML
                .replaceAll('__OPTION__', optionKey)
                .replaceAll('__ITEM__', itemKey)
                .trim();
            const itemRow = wrapper.firstElementChild;
            if (values.id) itemRow.insertAdjacentHTML('afterbegin', `<input type="hidden" name="bundle_options[${optionKey}][items][${itemKey}][id]" value="${values.id}">`);
            optionRow.querySelector('.bundle-items').appendChild(itemRow);
            setNestedRowValues(itemRow, values);
            initializeBundleSelect(itemRow.querySelector('.bundle-product-select'));
            if (String(values.deleted ?? '0') === '1') markDeleted(itemRow, '.bundle-item-deleted');
        }

        if (window.bundleOptionsOldInput) {
            Object.entries(window.bundleOptionsOldInput).forEach(function ([optionKey, values]) {
                const wrapper = document.createElement('div');
                wrapper.innerHTML = optionTemplate.innerHTML.replaceAll('__OPTION__', optionKey).trim();
                const optionRow = wrapper.firstElementChild;
                if (values.id) optionRow.insertAdjacentHTML('afterbegin', `<input type="hidden" name="bundle_options[${optionKey}][id]" value="${values.id}">`);
                document.getElementById('bundle-option-list').appendChild(optionRow);
                setNestedRowValues(optionRow, values);
                initializeBundleOptionFields(optionRow.querySelector('.bundle-option-fields'));
                Object.entries(values.items ?? {}).forEach(function ([itemKey, itemValues]) {
                    appendBundleItem(optionRow, optionKey, itemKey, itemValues);
                });
                if (String(values.deleted ?? '0') === '1') markDeleted(optionRow, '.bundle-option-deleted');
            });
        }

        document.getElementById('add-bundle-option')?.addEventListener('click', function () {
            const key = `new_${Date.now()}_${bundleOptionSequence++}`;
            const wrapper = document.createElement('div');
            wrapper.innerHTML = optionTemplate.innerHTML.replaceAll('__OPTION__', key).trim();
            const row = wrapper.firstElementChild;
            document.getElementById('bundle-option-list').appendChild(row);
            initializeBundleOptionFields(row.querySelector('.bundle-option-fields'));
        });

        bundleEditor?.addEventListener('click', function (event) {
            const removeOption = event.target.closest('.remove-bundle-option');
            if (removeOption) {
                markDeleted(removeOption.closest('.bundle-option-row'), '.bundle-option-deleted');
                return;
            }
            const removeItem = event.target.closest('.remove-bundle-item');
            if (removeItem) {
                markDeleted(removeItem.closest('.bundle-item-row'), '.bundle-item-deleted');
                return;
            }
            const addItem = event.target.closest('.add-bundle-item');
            if (addItem) {
                const optionRow = addItem.closest('.bundle-option-row');
                const optionKey = optionRow.dataset.optionKey;
                const itemKey = `new_${Date.now()}_${bundleItemSequence++}`;
                appendBundleItem(optionRow, optionKey, itemKey);
            }
        });
