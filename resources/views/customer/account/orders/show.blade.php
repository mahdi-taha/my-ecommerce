...
@section('account-content')
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
        ...
        <div class="d-flex flex-wrap gap-2">
            ...
            <a class="btn btn-secondary" href="{{ route('shop.account.orders.index') }}">
        ...
        <div class="card-body p-4">
            <div class="row g-3">
                <div class="col-md-4"><small
                        class="text-muted d-block">{{ __('shop.account.orders.order_number') }}</small><strong>{{ $order->order_number }}</strong>
                </div>
                <div class="col-md-4"><small
                        class="text-muted d-block">{{ __('shop.account.orders.order_date') }}</small><strong>{{ date('Y-m-d H:i', strtotime($order->placed_at)) }}</strong>
                </div>
                <div class="col-md-4"><small
                        class="text-muted d-block">{{ __('shop.account.orders.customer') }}</small><strong>{{ $order->customer_first_name }}
                        {{ $order->customer_last_name }}</strong></div>
                <div class="col-md-4"><small
                        class="text-muted d-block">{{ __('shop.account.orders.status') }}</small><strong>{{ __('shop.checkout.status.order.' . $order->status) }}</strong>
                </div>
                <div class="col-md-4"><small
                        class="text-muted d-block">{{ __('shop.checkout.confirmation.payment_status') }}</small><strong>{{ __('shop.checkout.status.payment.' . $order->payment_status) }}</strong>
                </div>
                <div class="col-md-4"><small
                        class="text-muted d-block">{{ __('shop.checkout.confirmation.fulfillment_status') }}</small><strong>{{ __('shop.checkout.status.fulfillment.' . $order->fulfillment_status) }}</strong>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body p-4">
            ...
            @foreach ($order->cancellationRequests as $cancellationRequest)
                <div class="border rounded p-3 mb-3">
                    ...
                    <p class="mb-1"><strong>{{ __('shop.account.orders.cancellation.reason') }}:</strong>
                        {{ $cancellationRequest->reason }}</p>
                    @if (in_array(
                            $cancellationRequest->status,
                            [\App\Enums\OrderCancellationRequestStatus::Approved, \App\Enums\OrderCancellationRequestStatus::Rejected],
                            true) && trim((string) $cancellationRequest->admin_note) !== '')
                        <p class="mb-0">
                            <strong>{{ __('shop.account.orders.cancellation.admin_note') }}:</strong>
            ...
            @if ($canRequestCancellation)
                <form method="POST"
                    action="{{ route('shop.account.orders.cancellation-requests.store', ['order' => $order]) }}">
                    @csrf
                    <div class="mb-3">
                        <label class="form-label"
                            for="cancellation-reason">{{ __('shop.account.orders.cancellation.reason') }}</label>
                    ...
                    <button class="btn btn-danger text-nowrap" type="submit" style="width: fit-content;">
                        ...
                        @if ($block['address'])
                            <address class="mb-0">
                                ...
                                @if ($block['address']->company)
                                    {{ $block['address']->company }}<br>
                                @endif
                                {{ $block['address']->address_line_1 }}<br>
                                @if ($block['address']->address_line_2)
                                    {{ $block['address']->address_line_2 }}<br>
                                @endif
                                {{ $block['address']->city }}@if ($block['address']->state)
                                    , {{ $block['address']->state }}
                                @endif
                                <br>
        ...
        <div class="card shadow-sm mb-4">
            <div class="card-body p-4">
                ...
                @foreach ($order->refunds as $refund)
                    <div class="border rounded p-3 mb-3">
                        <div class="d-flex justify-content-between"><strong>{{ $refund->refund_number }}</strong><small
                                class="text-muted">{{ $refund->refunded_at->format('Y-m-d H:i') }}</small></div>
                        <ul class="mt-2 mb-2">
                            @foreach ($refund->items as $item)
                                <li>{{ $item->orderItem->name }} — {{ (float) $item->quantity }}</li>
                            @endforeach
                        </ul>
                        <p class="mb-1"><strong>{{ __('shop.account.orders.refund_amount') }}:</strong>
                            {{ format_store_price($refund->customer_refund_amount, $refund->currency_code) }}</p>
                        @if ((float) $refund->shipping_deduction > 0)
                            <p class="mb-1"><strong>{{ __('shop.account.orders.shipping_deduction') }}:</strong>
                                {{ format_store_price($refund->shipping_deduction, $refund->currency_code) }}</p>
                        @endif
                        @if ($refund->customer_note)
                            <p class="mb-0"><strong>{{ __('shop.account.orders.refund_note') }}:</strong>
                                {{ $refund->customer_note }}</p>
                        @endif
                    </div>
                @endforeach
            </div>
                            ...
                            <tr>
                                <td>
                                    ...
                                    @foreach ($item->options as $option)
                                        <small class="d-block text-muted">{{ $option->attribute_name }}:
                                            {{ $option->option_label }}</small>
                                    @endforeach
                                </td>
                                <td>{{ $item->sku }}</td>
                                <td class="text-end">{{ (float) $item->quantity }}</td>
                                <td class="text-end">{{ format_store_price($item->unit_price, $order->currency_code) }}
                                </td>
            ...
            <div class="card shadow-sm">
                <div class="card-body p-4">
                    ...
                    <dl class="row g-2 mb-0">
                        ...
                        <dd class="col-5 text-end">{{ format_store_price($order->subtotal, $order->currency_code) }}</dd>
                        <dt class="col-7">{{ __('shop.checkout.tax') }}</dt>
                        <dd class="col-5 text-end">{{ format_store_price($order->tax_total, $order->currency_code) }}</dd>
                        <dt class="col-7">{{ __('shop.checkout.shipping') }}</dt>
                        <dd class="col-5 text-end">{{ format_store_price($order->shipping_total, $order->currency_code) }}
                        </dd>
                        <dt class="col-7 border-top pt-2">{{ __('shop.checkout.grand_total') }}</dt>
                        <dd class="col-5 border-top pt-2 text-end fw-bold">
                            {{ format_store_price($order->grand_total, $order->currency_code) }}</dd>
                    </dl>
                </div>
            </div>
        </div>
