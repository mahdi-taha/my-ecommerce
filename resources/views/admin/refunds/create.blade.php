<x-admin-main page="Create Refund">
    <x-slot name="header">@vite(['resources/css/app.css', 'resources/css/styles.min.css', 'resources/css/myStyle.css', 'resources/js/app.js', 'resources/js/admin/refunds.js'])</x-slot>
    <div class="page-wrapper" id="main-wrapper"><x-admin-sidebar /><div class="body-wrapper"><x-admin-topbar />
        <div class="container-fluid py-4"><h3>Create Refund</h3>
            @if(!$order)
                <section class="card card-body" data-refund-order-lookup data-lookup-url="{{ route('admin.refunds.lookups.orders') }}">
                    <label class="form-label" for="refund-order-search">Find an Order</label>
                    <input class="form-control" id="refund-order-search" type="search" autocomplete="off" placeholder="Search by Order Number, customer name, or email" aria-describedby="refund-order-search-help refund-order-search-status">
                    <div id="refund-order-search-help" class="form-text">Search by Order Number, customer name, or email.</div>
                    <div id="refund-order-search-status" class="small text-muted mt-3" role="status" aria-live="polite"></div>
