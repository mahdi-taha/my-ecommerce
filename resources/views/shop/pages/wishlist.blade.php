                            ...
                            <div class="row g-4 align-items-center">
                                ...
                                <div class="col-12 col-lg-2">
                                    ...
                                    @elseif ($isConfigurable)
                                        <span
                                            class="text-muted">{{ __('shop.product_details.select_options_for_price') }}</span>
                                    @endif
                                </div>
                                <div class="col-12 col-lg-4">
                                    <div class="d-flex justify-content-lg-end gap-2">
                                        @if ($productUrl)
                                            <a class="btn btn-secondary" style="white-space: nowrap;" href="{{ $productUrl }}">
                                                {{ __('shop.wishlist.view_product') }}
                                            </a>
                                        @endif

                                        <form method="POST"
                                            action="{{ route('shop.wishlist.destroy', ['product' => $product]) }}"
                                            class="d-inline">
                                            @csrf
                                            @method('DELETE')

                                            <button class="btn btn-danger text-nowrap" style="width: fit-content;" type="submit">
